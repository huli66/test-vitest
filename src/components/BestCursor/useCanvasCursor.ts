...
const useCanvasCursor = () => {
  ...
  // 渲染函数：每一帧更新和绘制所有线条（水墨风格）
  function render() {
    // 检查渲染是否正在运行
    if (ctx.running) {
      // 水墨风格：使用白色半透明填充来逐渐擦除黑色痕迹（模拟墨汁干涸）
      ctx.globalCompositeOperation = 'destination-out';
      // 使用极低透明度的白色填充，让之前的黑色痕迹逐渐变淡消失
  ...
  // 声明鼠标位置对象
  const pos = {},
    // 配置对象：包含所有可调参数（水墨风格优化）
    E = {
      // 调试模式开关
      debug: true,
      // 摩擦系数：控制线条运动的阻力，值越大阻力越大
      friction: 0.5,
      // 轨迹线数量：同时显示的线条数量（水墨风格：适当减少，避免过于浓重）
      trails: 15,
      // 每条线的节点数量：节点越多线条越平滑（水墨风格：增加节点使线条更流畅）
      size: 60,
      // 阻尼系数：控制节点间速度传递的衰减
      dampening: 0.25,
